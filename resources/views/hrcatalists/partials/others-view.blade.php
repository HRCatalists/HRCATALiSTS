...
<div class="border-top border-dark mb-5" id="section-others-{{ $employeeId }}">
    <div class="d-flex justify-content-between align-items-center">
        ...
        <div>
            <button type="button" class="btn btn-sm btn-outline-primary toggle-edit-btn" 
                data-section="others" data-employee-id="{{ $employeeId }}">
                Edit
            </button>
        </div>
    </div>
    ...
    <table class="table table-bordered text-center" id="othersTable-{{ $employeeId }}">
        ...
        <tbody>
            @foreach ($others as $i => $other)
                <tr>
                    <td>
                        <input type="date" name="others[{{ $i }}][date]" 
                               class="form-control section-field-others-{{ $employeeId }}" 
                               value="{{ $other->date }}" readonly>
                    </td>
                    <td>
                        <input type="text" name="others[{{ $i }}][description]" 
                               class="form-control section-field-others-{{ $employeeId }}" 
                               value="{{ $other->description }}" readonly>
                    </td>
    ...
    <button type="button" class="btn btn-sm btn-outline-success mb-3 d-none" id="addOtherBtn-{{ $employeeId }}">
        + Add Entry
    </button>
</div>