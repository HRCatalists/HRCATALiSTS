        ...
        <div id="content" class="flex-grow-1">
            <div class="container mt-5">
                ...
                @php
                    ...
                    $uniqueStatuses = $allApplicants->pluck('status')->unique()->values()->toArray();
                    $tabStatuses = array_merge(['all'], $uniqueStatuses); // Include 'all' as the first tab
                @endphp
{{-- 
                ...
                <ul class="nav nav-tabs mt-4" id="statusTabs" role="tablist">
                    @foreach ($tabStatuses as $key => $stat)
                        <li class="nav-item" role="presentation">
                            <button 
                                class="nav-link {{ $key === 0 ? 'active' : '' }}" 
                                id="{{ $stat }}-tab" 
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <span class="d-none d-print-block">
                                                    <span style="background-color: {{ $statusColors[$applicant->status] ?? '#000' }}; color: #fff; padding: 4px 8px; border-radius: 4px;">
                                                        {{ ucfirst($applicant->status) }}
                                                    </span>
                                                </span>
                                            
                                                <form method="POST" action="{{ route('applicants.chooseStatus', $applicant->id) }}" class="d-print-none">
                                                    @csrf
                    ...
                    @foreach ($tabStatuses as $key => $stat)
                        @php
                            $filteredApplicants = $stat === 'all'
                                ? $allApplicants->whereNotIn('status', ['hired', 'archived'])
                        ...
                        <div class="tab-pane fade {{ $key === 0 ? 'show active' : '' }}" 
                            ...
                            <table class="table table-bordered display applicantTable">
                                ...
                                <tbody>
                                                ...
                                                <form method="POST" action="{{ route('applicants.chooseStatus', $applicant->id) }}" class="d-print-none">
                                                    ...
                                                    <select name="status" class="form-select status-dropdown"
                                                        ...
                                                        @foreach ($tabStatuses as $s)
                                                            @if ($s !== 'all') {{-- prevent selecting 'all' as a status --}}
                                                                <option value="{{ $s }}" {{ $applicant->status == $s ? 'selected' : '' }}>
                                                                    {{ ucfirst($s) }}
                                                                </option>
                                                            @endif
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="text-center text-muted">No applicants under {{ ucfirst($stat) }}.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            function getVisibleCheckboxes() {
                const tab = getActiveTabPane();
                return [...tab.querySelectorAll(".rowCheckbox")];
            }

            function updateBulkButtons() {
                const anyChecked = getVisibleCheckboxes().some(cb => cb.checked);
            ...
            function getSelectedApplicantIds() {
                return getVisibleCheckboxes().filter(cb => cb.checked).map(cb => cb.value);
            }
            ...
            // === Select by Status (only in active tab)
            ...
            // === Individual Checkbox Tracking (live)
            ...
            updateBulkButtons(); // On first load
        });
    </script>
    {{-- <script>
        document.addEventListener("DOMContentLoaded", function () {
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            document.querySelector('.print-btn').addEventListener('click', function () {
                ...
                const tableToPrint = activeTab.querySelector('table');

                if (!tableToPrint) {
                    alert("No table found in the current tab.");
                    return;
                }

                // Clone table and styles
                ...
                const tableHTML = `
                    <html>
                        <head>
                            <title>Print - Applicant List</title>
                            ${styles}
                            <style>
                        ...
                        <body>
                            <h2>Applicant List - {{ ucfirst($tabStatuses[0]) }} Tab</h2>
